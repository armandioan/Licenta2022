...
public:
    ...
    bool gogogo() {
        WiFi.config(*ip, *gateway, *subnet, *dns1);  // 8888 sau 1111 sau 0000 sau gateway- dns pt a interoga time corect
...
void setup() {
    ...
    while (WiFi.status() != WL_CONNECTED) {
        if (index_r == sizeof(router) / sizeof(Router*) ) {
            hai_eeprom();  //nu s-a conectat deci porneste ap
        }
...
void read_eeprom() {
    for (uint8_t i = 0; i < 32; ++i) 
        esid += char(EEPROM.read(i)); //citest nume din eeprom char-by-char
...
void hai_eeprom() {                                 // incepe procedura de memorare a unei retele wifi fara hard coding, cu ajutorul memoriei eeprom a modulului
    ...
    while (1) {
        server1.handleClient(); // e atent daca se conecteaza cineva ap
    }
...
void loop() {
    if (Serial.available() > 0) {           // daca uC a trimis ceva (bloc standard) 
        fromQFP = Serial.readString();      // salvez ce a trimis 
    ...
    client.setTimeout(100);                     // ii setez timeout la 100ms ca nu dureaza mai mult sa trimit[ textul cu cererea
    if (client) {                               //daca am un client conectat
        ...
        while (client.connected() && currentTime - previousTime <= timeoutTime) {     //cat timp am client conectat si nu sta conectat degeaba
            ...
            if (client.available()) {                                                 //daca clientul a trimis un req
                ...
                //Serial.println(currentLine);//////////pt debug//////////////////////////////////////////////
            }
        }
        newd = true;
        firstline = true;
